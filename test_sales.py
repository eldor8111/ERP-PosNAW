...
def test_sales():
    ...
    try:
        ...
        print(f"Testing list_sales for user id: {user.id}, phone: {user.phone}, company_id: {user.company_id}")
        
        res = list_sales(skip=0, limit=20, db=db, current_user=user)
        print("Success! Number of sales:", len(res))
        
    except Exception as e:
        print("Exception during list_sales:")
        traceback.print_exc()
    finally:
        db.close()
