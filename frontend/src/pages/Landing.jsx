...
function DashboardMinimal() {
  ...
  return (
    <div className="bg-slate-900/70 backdrop-blur-xl border border-slate-700 rounded-[20px] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5),0_0_0_1px_rgba(255,255,255,0.1)] overflow-hidden transform [transform:rotateY(-8deg)_rotateX(4deg)_translateZ(0)] transition-transform duration-600 ease-[cubic-bezier(0.2,0.8,0.2,1)] hover:[transform:rotateY(0)_rotateX(0)_translateZ(30px)] hover:shadow-[0_35px_60px_-10px_rgba(0,0,0,0.2),0_0_40px_rgba(37,99,235,0.1)]">
      <div className="p-4 border-b border-black/[0.08] flex items-center gap-4 bg-slate-800/60">
        <div className="flex gap-2">
          <span className="w-3 h-3 rounded-full bg-red-500" />
          <span className="w-3 h-3 rounded-full bg-amber-500" />
          <span className="w-3 h-3 rounded-full bg-emerald-500" />
        </div>
        <div className="bg-slate-900 border border-black/[0.08] rounded-lg px-4 py-1.5 text-xs text-slate-400 flex-1">🔍 Qidiruv...</div>
      </div>
      <div className="flex h-[360px]">
        <div className="w-[70px] border-r border-black/[0.08] p-4 flex flex-col gap-4 bg-slate-900/40">
          ...
          <div className="h-7 rounded-lg bg-slate-800 transition-all duration-300 hover:scale-105" />
          <div className="h-7 rounded-lg bg-slate-800 transition-all duration-300 hover:scale-105" />
          <div className="h-7 rounded-lg bg-slate-800 transition-all duration-300 hover:scale-105" />
        </div>
        <div className="flex-1 p-6 flex flex-col gap-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-slate-900 border border-black/[0.08] p-4 rounded-xl shadow-sm transition-transform duration-300 hover:-translate-y-1">
              <span className="block text-[11px] text-slate-400 uppercase font-bold tracking-[1px]">{t('dashboard.income')}</span>
              <span className="block text-[22px] font-black text-white mt-1.5">12.4M</span>
            </div>
            <div className="bg-slate-900 border border-black/[0.08] p-4 rounded-xl shadow-sm transition-transform duration-300 hover:-translate-y-1">
              <span className="block text-[11px] text-slate-400 uppercase font-bold tracking-[1px]">{t('sidebar.customers')}</span>
              <span className="block text-[22px] font-black text-white mt-1.5">342</span>
            </div>
            <div className="bg-slate-900 border border-black/[0.08] p-4 rounded-xl shadow-sm transition-transform duration-300 hover:-translate-y-1">
              <span className="block text-[11px] text-slate-400 uppercase font-bold tracking-[1px]">{t('purchase.order')}</span>
              <span className="block text-[22px] font-black text-white mt-1.5">1,890</span>
            </div>
          </div>
          <div className="flex-1 bg-slate-900 border border-black/[0.08] rounded-xl p-5 flex items-end gap-3">
...
function TabbedModules({ t }) {
  ...
  return (
    <div className="flex gap-10 bg-slate-900/80 backdrop-blur-2xl border border-slate-700 rounded-3xl p-10 shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
      <div className="flex flex-col gap-4">
        {tabs.map((tab, idx) => (
          <button
            key={idx}
            className={`text-left w-full bg-slate-900 overflow-hidden border p-6 rounded-[20px] cursor-pointer transition-all duration-300 shadow-sm group ${activeTab === idx
              ? 'border-blue-600/30 h-40 shadow-[0_10px_30px_rgba(37,99,235,0.1),inset_3px_0_0_#2563eb]'
              : 'border-black/8 h-20 hover:bg-slate-800'
              }`}
            onClick={() => { setActiveTab(idx) }}
          >
            <div className='flex justify-between items-center'>
              <h3 className={`m-0 mb-2 text-[20px] font-bold flex items-center gap-2 transition-all duration-300 ${activeTab === idx ? 'text-blue-600' : 'text-slate-400'}`}>
                {oneDbIcons[idx]}
                {tab.title}
              </h3>

              <ChevronDown className={`-translate-y-0.75 transition-all duration-300 ease-in-out ${activeTab === idx ? 'rotate-180 text-blue-600' : 'rotate-0 text-slate-400'}`} />
            </div>
            <p className={`m-0 text-[15px] transition-all duration-300 leading-[1.6] ${activeTab === idx ? 'text-slate-400' : 'text-white'}`}>{tab.desc}</p>
          </button>
        ))}
      </div>
  ...
  const modules = [
    {
      ...
      desc: t('erp.m1.d') || 'Bir nechta kassa punktlarini yagona tizimga ulang. Shtrix-kod, skanerlash, naqd va bank kartasi to\'lovlari avtomatlashtirilgan.',
      features: [
    ...
    },
    {
      ...
      desc: t('erp.m4.d') || 'Kassa harakati, xarajatlar, daromadlar va foyda/zarar (P&L) hisobotlarini real-vaqtda nazorat qiling.',
      features: [
    ...
    },
    {
      ...
      desc: t('erp.m6.d') || 'Kassir, menejer, ombordor, direktor — har bir xodimga alohida ruxsatlar bering. To\'liq audit log bilan operatsiyalarni kuzating.',
      features: [
  ...
  return (
    <LandingLayout>
      ...
      <header className="relative pt-[100px] mt-1 pb-[120px] border-b border-black/[0.02]">
        <div className="absolute inset-0 bg-[linear-gradient(to_right,rgba(255,255,255,0.05)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.05)_1px,transparent_1px)] bg-[size:40px_40px] [mask-image:radial-gradient(ellipse_at_top,black_30%,transparent_80%)] z-0" />
        <div className="max-w-[1400px] mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-[60px] items-center text-center lg:text-left">
          <div className="flex flex-col items-center lg:items-start">
            ...
            <h1 className="animate-[slideUp_0.8s_ease-out_0.2s_backwards] text-[clamp(32px,9vw,55px)] lg:text-[clamp(48px,6vw,68px)] font-black leading-[1.05] tracking-[-0.04em] m-0 mb-6 text-white">
              {t('land.erp')} <br />
            ...
            <p className="animate-[slideUp_0.8s_ease-out_0.3s_backwards] text-[19px] text-slate-400 leading-[1.6] m-0 mb-10 max-w-[560px]">
              {t('land.desc1')}
            </p>
      ...
      <section id="features" className="py-[120px] relative">
        <div className="max-w-[1400px] mx-auto relative z-10">
          <div className="text-center mb-[70px] animate-[fadeInUp_0.8s_ease-out_both]">
            <h2 className="text-[clamp(32px,5vw,44px)] font-extrabold tracking-[-0.04em] mb-4 text-white">{t('land.ecosystem')}</h2>
            <p className="text-[18px] text-slate-400 max-w-[600px] mx-auto">{t('land.arch')}</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 auto-rows-[minmax(280px,auto)] gap-6">
            ...
            >
              <div className="relative z-10 flex flex-col h-full transition-transform duration-300 group-hover:-translate-y-1">
                <div className="w-14 h-14 bg-slate-900 border border-black/8 rounded-2xl flex items-center justify-center mb-6 text-blue-600 shadow-sm transition-all duration-400 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] group-hover:scale-[1.15] group-hover:rotate-[5deg] group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 group-hover:shadow-[0_10px_20px_rgba(37,99,235,0.3)]">
                  {ICONS.pos}
                </div>
                <h3 className="m-0 mb-4 text-[24px] font-extrabold tracking-[-0.02em] text-white">{t('land.omni')}</h3>
                <p className="m-0 text-slate-400 leading-[1.7] text-[16px]">{t('land.omniDesc')}</p>
            ...
            <div
              className="group cursor-pointer bg-slate-900/70 backdrop-blur-lg border border-slate-700/80 rounded-[24px] p-10 flex flex-col relative overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.04),inset_0_0_0_1px_rgba(255,255,255,0.5)] transition-all duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)] animate-[slideUp_0.8s_backwards_0.2s] hover:-translate-y-2 hover:scale-[1.01] hover:shadow-[0_30px_60px_rgba(0,0,0,0.08),inset_0_0_0_2px_rgba(37,99,235,0.1)]"
              onClick={() => navigate('/erp-tizim')}
              title="Analitika va hisobotlar"
            >
              <div className="relative z-10 flex flex-col h-full transition-transform duration-300 group-hover:-translate-y-1">
                <div className="w-14 h-14 bg-slate-900 border border-black/8 rounded-2xl flex items-center justify-center mb-6 text-blue-600 shadow-sm transition-all duration-400 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] group-hover:scale-[1.15] group-hover:rotate-[5deg] group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 group-hover:shadow-[0_10px_20px_rgba(37,99,235,0.3)]">{ICONS.chart}</div>
                <h3 className="m-0 mb-4 text-[24px] font-extrabold tracking-[-0.02em] text-white">{t('land.bi')}</h3>
                <p className="m-0 text-slate-400 leading-[1.7] text-[16px]">{t('land.biDesc')}</p>
            ...
            >
              <div className="relative z-10 flex flex-col h-full transition-transform duration-300 group-hover:-translate-y-1">
                ...
                <h3 className="m-0 mb-4 text-[24px] font-extrabold tracking-[-0.02em] text-white">{t('land.crm')}</h3>
                <p className="m-0 text-slate-400 leading-[1.7] text-[16px]">{t('land.crmDesc')}</p>
                <span className="inline-flex items-center gap-1.5 text-[14px] font-bold text-blue-600 mt-5 opacity-0 -translate-x-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0">{t('land.chaqqon.box_more') || 'Batafsil'} {ICONS.arrowRight}</span>
              </div>
            </div>
            ...
            >
              <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-[30px] lg:gap-[40px] h-full transition-transform duration-300 group-hover:-translate-y-1">
                <div>
                  <div className="w-14 h-14 bg-slate-900 border border-black/[0.08] rounded-2xl flex items-center justify-center mb-6 text-blue-600 shadow-sm transition-all duration-400 ease-[cubic-bezier(0.175,0.885,0.32,1.275)] group-hover:scale-[1.15] group-hover:rotate-[5deg] group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 group-hover:shadow-[0_10px_20px_rgba(37,99,235,0.3)]">{ICONS.warehouse}</div>
                  <h3 className="m-0 mb-4 text-[24px] font-extrabold tracking-[-0.02em] text-white">{t('land.wms')}</h3>
                  <p className="m-0 text-slate-400 leading-[1.7] text-[16px]">{t('land.wmsDesc')}</p>
                ...
                <ul className="list-none p-0 m-0 flex flex-col gap-4 bg-slate-900 p-8 rounded-[20px] border border-black/[0.08] min-w-0 lg:min-w-[300px] w-full lg:w-auto shadow-sm transform translate-z-0 transition-all duration-300 group-hover:scale-[1.02] group-hover:shadow-md">
                  <li className="flex items-center gap-3 text-[15px] font-semibold text-white group/item">{ICONS.check} {t('land.check1')}</li>
                  <li className="flex items-center gap-3 text-[15px] font-semibold text-white group/item">{ICONS.check} {t('land.check2')}</li>
                  <li className="flex items-center gap-3 text-[15px] font-semibold text-white group/item">{ICONS.check} {t('land.check3')}</li>
                </ul>
              </div>
            </div>
      ...
      <section className="py-[100px] max-w-[1400px] mx-auto" id="modullar">
        <div>
          <div className="text-center mb-[72px]">
            ...
            <h2 className="text-3xl md:text-[4vw] lg:text-[44px] font-black text-white tracking-tight mb-4">{t('erp.mod.title') || '6 ta kuchli modul — bitta tizim'}</h2>
            <p className="text-lg text-slate-400 max-w-[560px] mx-auto leading-relaxed">{t('erp.mod.desc') || 'Har bir biznes jarayoni uchun maxsus ishlab chiqilgan, bir-biri bilan to\'liq integratsiya qilingan modullar'}</p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6 bg-slate-900 rounded-3xl border border-black/5 overflow-hidden shadow-2xl shadow-black/5">
            {/* Sidebar */}
            <div className="border-r border-black/5 flex justify-between lg:flex-col w-full bg-slate-800/50 p-2 gap-1">
              {modules.map((mod, idx) => (
                <button
                  key={mod.id}
                  className={`flex cursor-pointer items-center gap-4 p-5 rounded-xl border-1.5 border-transparent transition-all text-left font-sans ${activeModule === idx
                    ...
                    : 'text-slate-400 hover:bg-black/5 hover:text-white'
                    }`}
            ...
            <div className="p-10 animate-fadeInUp" key={activeModule}>
              <div className={`flex items-center gap-5 mb-6 pb-6 border-b-2 ${modules[activeModule].borderClass}`}>
                ...
                <div>
                  <h3 className="text-2xl font-black text-white tracking-tight">{modules[activeModule].title}</h3>
                  <p className="text-sm text-slate-400 mt-1">{modules[activeModule].subtitle}</p>
                </div>
              </div>
              <p className="text-[17px] leading-relaxed text-slate-300 mb-8">{modules[activeModule].desc}</p>

              <div className="flex gap-4 mb-8 flex-wrap">
                {modules[activeModule].stats.map((s, i) => (
                  <div key={i} className={`flex-1 min-w-[140px] bg-slate-800 border border-black/5 border-t-3 rounded-xl p-4 text-center ${modules[activeModule].borderClass}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-8">
                {modules[activeModule].features.map((f, i) => (
                  <div key={i} className="flex items-center gap-3 text-[15px] font-semibold text-slate-100">
      ...
      <section id="modules" className="py-[120px] relative bg-gradient-to-b from-slate-100 to-white border-y border-black/[0.08]">
        <div className="max-w-[1400px] mx-auto relative z-10">
          <div className="text-center mb-[70px] animate-[fadeInUp_0.8s_ease-out_both]">
            <h2 className="text-[clamp(32px,5vw,44px)] font-extrabold tracking-[-0.04em] mb-4 text-white">{t('land.singleDb')}</h2>
            <p className="text-[18px] text-slate-400 max-w-[600px] mx-auto">{t('land.noPaper')}</p>
          </div>
          <TabbedModules t={t} />
        </div>
      </section>
      ...
      <section className="py-[120px] relative bg-slate-800 border-t border-slate-700">
        <div className="max-w-[1400px] mx-auto relative z-10">
          <div className="text-center mb-[70px] animate-[fadeInUp_0.8s_ease-out_both]">
            <h2 className="text-[clamp(32px,5vw,44px)] font-extrabold tracking-[-0.04em] mb-4 text-white">{t('land.agency.title') || 'Biz bilan faqat ERP emas...'}</h2>
            <p className="text-[18px] text-slate-400 max-w-[600px] mx-auto">{t('land.agency.sub')}</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">

            {/* Veb-saytlar */}
            <div
              className="group cursor-pointer bg-slate-900 text-white border border-slate-700 rounded-2xl p-7 shadow-[0_4px_6px_rgba(0,0,0,0.02)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_24px_rgba(0,0,0,0.08)] hover:border-blue-600"
            ...
            >
              <div className="flex flex-col h-full relative z-10">
                ...
                <p className="text-slate-300 text-[1rem] leading-[1.5]">{t('land.agency.w_desc')}</p>
            ...
            >
              <div className="flex flex-col h-full relative z-10">
                ...
                <p className="text-slate-300 text-[1rem] leading-[1.5]">{t('land.agency.b_desc')}</p>
                <span className="inline-block text-[14px] font-bold text-blue-600 mt-3.5 opacity-0 translate-y-1.5 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">{t('land.chaqqon.box_more')} →</span>
              </div>
            </div>
            ...
            >
              <div className="flex flex-col h-full relative z-10">
                ...
                <p className="text-slate-300 text-[1rem] leading-[1.5]">{t('land.agency.s_desc')}</p>
                <span className="inline-block text-[14px] font-bold text-blue-600 mt-3.5 opacity-0 translate-y-1.5 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">{t('land.chaqqon.box_more')} →</span>
              </div>
            </div>
      ...
      <section id="lead-form" className="py-[140px] bg-blue-600 relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto relative z-10">
          <div className="bg-slate-900/10 text-white backdrop-blur-[20px] border border-slate-700/20 rounded-[24px] shadow-[0_30px_60px_rgba(0,0,0,0.2)] relative z-[1] grid grid-cols-1 lg:grid-cols-2 gap-10 text-center lg:text-left p-[30px] lg:p-[60px]">
              ...
              {leadStatus === 'success' ? (
                ...
                <form onSubmit={handleLeadSubmit} className="flex w-full flex-col gap-[15px] text-left">
                  ...
                  <div className="flex flex-col">
                    ...
                    >
                      <label className="text-[0.9rem] text-slate-300 block mb-2">{t('land.form.service')}</label>

                      <div className="relative">
                        <ListboxButton className="w-full cursor-pointer flex items-center p-3 justify-between rounded-lg border border-slate-700 text-[1rem] bg-slate-900 text-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">
                          <span className="flex items-center gap-3">
                            {/* Tanlangan matnga qarab ikonkani komponent sifatida yaratamiz */}
                            {(() => {
                              const SelectedIcon = getServiceIcon(leadForm.service);
                              return <SelectedIcon className="size-5 shrink-0 text-slate-400" />;
                            })()}
                        ...
                        <ListboxOptions
                          transition
                          className="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md outline-0 bg-slate-900 text-base border border-slate-700 transition duration-100 ease-in data-[closed]:opacity-0 sm:text-sm shadow-lg"
                        >
                          {serviceOptions.map((optionName, index) => {
                            ...
                            return (
                              <ListboxOption
                                key={index}
                                value={optionName}
                                className="group relative py-2.5 pr-9 pl-3 select-none cursor-pointer text-slate-100 data-[focus]:bg-blue-500 data-[focus]:text-white outline-hidden"
                              >
                  ...
                  <div className="flex flex-col">
                    <label className="text-[0.9rem] text-slate-300 mb-2 block">{t('land.form.name')}</label>
                    <input
                      type="text"
                      required
                      value={leadForm.name}
                      ...
                      placeholder="Alisher"
                      className="w-full p-3 rounded-lg border border-slate-700 text-[1rem] bg-slate-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors"
                    />
                  </div>

                  <div className="flex flex-col">
                    <label className="text-[0.9rem] text-slate-300 mb-2 block">{t('land.form.phone')}</label>
                    <input
                      type="text"
                      required
