                      ...
                      .map(c => (
                        <tr key={c.id} className={`transition-colors ${c.overdue ? 'bg-red-50/50 hover:bg-red-50' : 'hover:bg-slate-50'}`}>
                          ...
                          <td className="px-6 py-4">
                            <button
                              onClick={() => {
                                setPayModal({ type: 'customer', id: c.id, name: c.name, balance: c.debt_balance, balances: c.debt_balances });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4">
            ...
            <p className="text-sm text-slate-500 mb-4">
              {payModal.name} — {t('finance.currentDebt') || 'joriy qarz'}: <strong className="text-amber-600">
                {payModal.balances && Object.keys(payModal.balances).length > 0 
                  ? Object.entries(payModal.balances).map(([curr, amt]) => `${Number(amt || 0).toLocaleString('uz-UZ')} ${curr === 'UZS' ? "so'm" : curr}`).join(' + ')
                  : fmt(payModal.balance, t)}
              </strong>
            </p>
            <div className="mb-4">
              <label className="block text-xs font-medium text-slate-600 mb-1.5">{t('finance.paymentAmount') || "To'lov summasi"} ({payCurrency === 'UZS' ? t('common.sum') || "so'm" : payCurrency})</label>
              <div className="flex gap-2">
