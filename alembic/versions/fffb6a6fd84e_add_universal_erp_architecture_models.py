...
def upgrade() -> None:
    ...
    has_attr = any(c['name'] == 'attributes' for c in insp.get_columns('products')) if 'products' in tables else True
    if not has_attr:
        op.add_column('products', sa.Column('attributes', sa.JSON(), nullable=True))
        
    has_tags = any(c['name'] == 'tags' for c in insp.get_columns('products')) if 'products' in tables else True
    if not has_tags:
        op.add_column('products', sa.Column('tags', sa.JSON(), nullable=True))
        
    op.execute("ALTER TABLE products ALTER COLUMN product_type TYPE VARCHAR(20)")
